...
function resolveSlot(slot, winners) {
  ...
  if (slot.from) {
    ...
    return result === "A" ? resolveSlot(prev.a, winners)
                          : resolveSlot(prev.b, winners);
  }
...
function songLabel(key) {
  if (!key) return "Waiting on previous result…";
  const s = SONGS[key];
